...
(require "ast.rkt" "interp.rkt" "compile.rkt" a86/interp)

(define (interp-compile-match e)
  (equal? (asm-interp (compile e)) (interp e)))
...
(define (shrink-term e)
  (match e
    ...
    [(IfZero e1 e2 e3)
     (append (list e2 e3)
             (map (lambda (es) (IfZero es e2 e3)) (shrink-term e1))
             (map (lambda (es) (IfZero e1 es e3)) (shrink-term e2))
             (map (lambda (es) (IfZero e1 e2 es)) (shrink-term e3)))]))
