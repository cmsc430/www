...
val_t* val_unwrap(val_t v) {
  return (val_t*)((v >> imm_shift) << imm_shift);
}

int64_t type_tag(type_t t) {
  switch (t) {
  case T_BOX:
    return box_type_tag;
  case T_CONS:
    return cons_type_tag;
  case T_STR:
    return str_type_tag;
  case T_VECT:
    return vect_type_tag;
  default:
    printf("type_tag called on non-pointer type");
    exit(1);
  }
}

int val_size(val_t *v, type_t t) {
  switch (t) {
  case T_CONS: return 2;
  case T_VECT: return 1 + v[0];
  case T_STR:  return 1 + ((v[0] + 1) / 2);
  default: return 1;
  }
};

val_t val_wrap(val_t* v, type_t t) {
  return (val_t)((int64_t)v ^ type_tag(t));
}
